/*
 * Project: Voice Reminder
 * Author: Wei Lee, kevin_jia@foxmail.com
 * Copyright: (C) 2022 by Wei
 * License: GNU GPL v3 (see License.txt)
 */
#include "common.h"
#include "main.h"

#define OPTION_COUNT					6

#define PARAM_X								6//
#define SETTING_NOW_NONE			0
#define SETTING_NOW_ODDEVEN		1
#define SETTING_NOW_START_Y		2
#define SETTING_NOW_START_M		3
#define SETTING_NOW_START_D		4
#define SETTING_NOW_END_Y			5
#define SETTING_NOW_END_M			6
#define SETTING_NOW_END_D			7

//static driverInfo_s driverInfo;
static prev_menu_s prevMenuData;
static void mSelect(void);
static void itemLoader(byte num);
static void mDown(void);
static void mUp(void);
static void showOddEvenStr(void);
static void showDaysleftStr(void);
static void selectOddEvenNum(void);
static void selectDaysleftNum(void);
void checkDataValid(void);
void paramUpdate(void);

void modeOddevenSelect()
{
	setMenuInfo(OPTION_COUNT, MENU_TYPE_STR, PSTR(STR_LIMITODDOREVEN));
	setMenuFuncs(mDown, mSelect, mUp, itemLoader);

	setPrevMenuOpen(&prevMenuData, modeOddevenSelect);
	//1.从flash读取当前的单双号模式和退出时间
	static unsigned int sizeStruct = 0;
	sizeStruct = sizeof(driverInfo_s);
	readFlash(START_FLASH_ADDRESS, (uint16_t *)driverInfo_P, sizeStruct);
	driverInfo_P->mode = 2;
	checkDataValid();
	menuData.selected = 1;	
	beginAnimation2(NULL);
}

static void mDown()
{
	nextOption();
	// Some lines are blank, skip them
	if(menuData.selected == 0 || menuData.selected == 2)
		nextOption();
	if(menuData.selected == 3)
		nextOption();
	if(menuData.selected == 4)
		nextOption();
}

static void mUp()
{
	prevOption();
	if(menuData.selected == 4)
		prevOption();
	if(menuData.selected == 3)
		prevOption();
	// Some lines are blank, skip them
	if(menuData.selected == 0 || menuData.selected == 2 || menuData.selected == 4)
		prevOption();
}

static void mSelect()
{
	bool isExiting = exitSelected();
	if(isExiting)
	{
		//save rules to flash	
		unsigned int ret = 99;
		readFlash(START_FLASH_ADDRESS, (uint16_t *)flashCont, sizeof(driverInfo_s));
		if(flashCont->limitDS.normal != driverInfo_P->limitDS.normal)
			wFlash(START_FLASH_ADDRESS, (uint16_t *)driverInfo_P, sizeof(driverInfo_s));
		else
		{
			ret = memcmp(flashCont->limitDS.endYMD, driverInfo_P->limitDS.endYMD, \
			sizeof(flashCont->limitDS.endYMD));
			if(ret != 0)
				wFlash(START_FLASH_ADDRESS, (uint16_t *)driverInfo_P, sizeof(driverInfo_s));
		}
		if(flashCont->mode != driverInfo_P->mode)
			wFlash(START_FLASH_ADDRESS, (uint16_t *)driverInfo_P, sizeof(driverInfo_s));
	}
	setPrevMenuExit(&prevMenuData);
	doAction(exitSelected());	
}

static void itemLoader(byte num)
{
	UNUSED(num);
	showOddEvenStr();
	showDaysleftStr();
	addBackOption();
}

static void showOddEvenStr(void)
{
	char buff[14] = {0};
	byte fontArr[6] = {16, 18, 16, 15, 19, 7};
	
	if(driverInfo_P->limitDS.normal == 0)
		fontArr[0] = 16;//单
	else
		fontArr[0] = 17;//双
	LOOP(6, cnt)
	{			
		draw_bitmap(PARAM_X+cnt*16, 8, textDisplay[fontArr[cnt]], 16, 16, NOINVERT, 0);
	}	
	setMenuOption(1, buff, NULL, selectOddEvenNum);
}

static void showDaysleftStr(void)		//????N??????????
{
	char buff[14] = {0};
	byte fontArr[4] = {27, 28, 25, 26};
	byte xArr[10] = {6, 6+11,  6+11+11+8, 6+11+11+8+11, 6+11+11+8+11+11+8, 6+11+11+8+11+11+8+11};
	byte highB = 0, lowB = 0, forMonth = 0;
	LOOP(6, cnt)					//draw 6 start year, month, day
	{	
		forMonth = driverInfo_P->limitDS.startYMD[cnt/2];
		if(cnt == 2 || cnt == 3)//month need plus 1
			forMonth += 1;
		highB = forMonth / 10;
		lowB = forMonth % 10;
		draw_bitmap(xArr[cnt], 	 8+16, numFont11x16[highB], 11, 16, NOINVERT, 0);	
		draw_bitmap(xArr[cnt+1], 8+16, numFont11x16[lowB],  11, 16, NOINVERT, 0);	
		cnt += 1;
	}
	LOOP(6, cnt)					//draw 6 end year, month, day
	{	
		forMonth = driverInfo_P->limitDS.endYMD[cnt/2];
		if(cnt == 2 || cnt == 3)//month need plus 1
			forMonth += 1;
		highB = forMonth / 10;
		lowB = forMonth % 10;
		draw_bitmap(xArr[cnt], 	 8+16+16, numFont11x16[highB], 11, 16, NOINVERT, 0);	
		draw_bitmap(xArr[cnt+1], 8+16+16, numFont11x16[lowB],  11, 16, NOINVERT, 0);	
		cnt += 1;
	}
	LOOP(32, y)						//draw 4 point
	{
		draw_bitmap(28, 8+16+y, numFont11x16[11], 8, 16, NOINVERT, 0);	
		draw_bitmap(58, 8+16+y, numFont11x16[11],  8, 16, NOINVERT, 0);
		y += 16;
	}

	LOOP(4, cnt)				//draw Begin and Exit 4 words
	{
		draw_bitmap(88+cnt%2*16, 8+16+cnt/2*16, textDisplay[fontArr[cnt]], 16, 16, NOINVERT, 0);
	}		

	setMenuOption(5, buff, NULL, selectDaysleftNum);
}

static display_t oddEvenNumDraw()		//???&???????
{
	byte x = PARAM_X;
	byte y = 24;   //0  1    2         3         4              5
	byte w = 16;   //2   2    .        0          8             .
	byte xArr[10] = {6, 6+11, 6+11+11, 6+11+11+8, 6+11+11+8+11, 6+11+11+8+11+11,\
				  //3				   0                     ??88                     ?104 
					6+11+11+8+11+11+8, 6+11+11+8+11+11+8+11, 6+11+11+8+11+11+8+11+11, 6+11+11+8+11+11+8+11+11+16};
	byte highB = 0, lowB = 0, month = 0;
	switch(setting.now)
	{
		case SETTING_NOW_ODDEVEN:	
			y = 8;	
			draw_clearArea(x, y, w);
			draw_clearArea(x, y+8, w);
			draw_bitmap(x, y, textDisplay[driverInfo_P->limitDS.normal+16] , 16, 16, INVERT, 0);//
			break;
		case SETTING_NOW_START_Y:
			y = 8+16;
			highB = driverInfo_P->limitDS.startYMD[0] / 10;
			lowB = driverInfo_P->limitDS.startYMD[0] % 10;
			draw_clearArea(xArr[0], y, 22);
			draw_clearArea(xArr[0], y+8, 22);
			draw_bitmap(xArr[0], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[1], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;
		case SETTING_NOW_START_M:
			y = 8+16;	
			month	= driverInfo_P->limitDS.startYMD[1] + 1;	
			highB = month / 10;
			lowB = month % 10;
			draw_clearArea(xArr[3], y, 22);
			draw_clearArea(xArr[3], y+8, 22);
			draw_bitmap(xArr[3], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[4], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;
		case SETTING_NOW_START_D:
			y = 8+16;	
			highB = driverInfo_P->limitDS.startYMD[2] / 10;
			lowB = driverInfo_P->limitDS.startYMD[2] % 10;
			draw_clearArea(xArr[6], y, 22);
			draw_clearArea(xArr[6], y+8, 22);
			draw_bitmap(xArr[6], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[7], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;
		case SETTING_NOW_END_Y:
			y = 8+16+16;	
			highB = driverInfo_P->limitDS.endYMD[0] / 10;
			lowB = driverInfo_P->limitDS.endYMD[0] % 10;
			draw_clearArea(xArr[0], y, 22);
			draw_clearArea(xArr[0], y+8, 22);
			draw_bitmap(xArr[0], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[1], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;
		case SETTING_NOW_END_M:
			y = 8+16+16;	
			month	= driverInfo_P->limitDS.endYMD[1] + 1;	
			highB = month / 10;
			lowB = month % 10;
			draw_clearArea(xArr[3], y, 22);
			draw_clearArea(xArr[3], y+8, 22);
			draw_bitmap(xArr[3], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[4], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;
		case SETTING_NOW_END_D:
			y = 8+16+16;	
			highB = driverInfo_P->limitDS.endYMD[2] / 10;
			lowB = driverInfo_P->limitDS.endYMD[2] % 10;
			draw_clearArea(xArr[6], y, 22);
			draw_clearArea(xArr[6], y+8, 22);
			draw_bitmap(xArr[6], y, numFont11x16[highB] , 11, 16, INVERT, 0);
			draw_bitmap(xArr[7], y, numFont11x16[lowB] , 11, 16, INVERT, 0);
			break;			
		default:
			break;
	}

	return DISPLAY_DONE;
}

static void paramDataUp()
{
	static u16 days = 0;
	setting.val++;
	paramUpdate();
	days = formatDaysCounter(driverInfo_P->limitDS.startYMD, driverInfo_P->limitDS.endYMD);
	//driverInfo_P->limitDS.normal = setting.val;			//?????????????????????????
}

static void paramDataDown()
{
	static u16 days = 0;
	setting.val--;
	paramUpdate();
	days = formatDaysCounter(driverInfo_P->limitDS.startYMD, driverInfo_P->limitDS.endYMD);
	//driverInfo_P->limitDS.normal = setting.val;			//?????????????????????????
}

static void beginSelect()
{
	setMenuFuncs(paramDataUp, mSelect, paramDataDown, itemLoader);
	menuData.func.draw = oddEvenNumDraw;
}

static void endSelect()
{
	setting.now = SETTING_NOW_NONE;
	setMenuFuncs(mDown, mSelect, mUp, itemLoader);
	menuData.func.draw = NULL;
}

static void selectOddEvenNum()
{
	beginSelect();	
	
	switch(setting.now)
	{
		case SETTING_NOW_NONE:
			setting.now = SETTING_NOW_ODDEVEN;
			setting.val = driverInfo_P->limitDS.normal;
			break;
		case SETTING_NOW_ODDEVEN:
			setting.val = driverInfo_P->limitDS.normal;				
			endSelect();		
			break;
		default:
			setting.now = SETTING_NOW_NONE;
			endSelect();
			break;
	}	
}
static void selectDaysleftNum()
{
	beginSelect();	
	
	switch(setting.now)
	{
		case SETTING_NOW_NONE:
			setting.now = SETTING_NOW_START_Y;
			setting.val = driverInfo_P->limitDS.startYMD[0];
			break;
		case SETTING_NOW_START_Y:
			setting.now = SETTING_NOW_START_M;
			setting.val = driverInfo_P->limitDS.startYMD[1];
			break;
		case SETTING_NOW_START_M:	
			setting.now = SETTING_NOW_START_D;		
			setting.val = driverInfo_P->limitDS.startYMD[2];
			break;
		case SETTING_NOW_START_D:
			setting.now = SETTING_NOW_END_Y;
			setting.val = driverInfo_P->limitDS.endYMD[0];
			break;
		case SETTING_NOW_END_Y:
			setting.now = SETTING_NOW_END_M;
			setting.val = driverInfo_P->limitDS.endYMD[1];
			break;
		case SETTING_NOW_END_M:
			setting.now = SETTING_NOW_END_D;
			setting.val = driverInfo_P->limitDS.endYMD[2];
			break;
		case SETTING_NOW_END_D:
			setting.val = driverInfo_P->limitDS.endYMD[2];
			endSelect();
			break;
		default:
			setting.now = SETTING_NOW_NONE;
			endSelect();
			break;
	}	
}

void paramUpdate(void)
{
	byte daysOfMonth = 0;
	switch(setting.now)
	{
		case SETTING_NOW_ODDEVEN:
			driverInfo_P->limitDS.normal ? (driverInfo_P->limitDS.normal = 0) : (driverInfo_P->limitDS.normal = 1);
			break;
		case SETTING_NOW_START_Y:
			if(setting.val > 99)
				setting.val = 22;
			else if(setting.val < 22)
				setting.val = 99;
			driverInfo_P->limitDS.startYMD[0] = setting.val;
			break;
		case SETTING_NOW_START_M:
			if(setting.val == 0xFF)
				setting.val = 11;
			else if(setting.val > 11)
				setting.val = 0;
			driverInfo_P->limitDS.startYMD[1] = setting.val;
			//every change calculate days of this 			
			daysOfMonth = time_monthDayCount((month_t)driverInfo_P->limitDS.startYMD[1], \
				driverInfo_P->limitDS.startYMD[0]);
			if(driverInfo_P->limitDS.startYMD[2] > daysOfMonth)
				driverInfo_P->limitDS.startYMD[2] = daysOfMonth;
			break;
		case SETTING_NOW_START_D:			
			daysOfMonth = time_monthDayCount((month_t)driverInfo_P->limitDS.startYMD[1], \
				driverInfo_P->limitDS.startYMD[0]);
			if(setting.val > daysOfMonth)		//Todo:need replace 31
				setting.val = 1;
			else if(setting.val < 1)
				setting.val = daysOfMonth;		//Todo:need replace 31
			driverInfo_P->limitDS.startYMD[2] = setting.val;
			break;		
		case SETTING_NOW_END_Y:		//need larger than start year
			if(setting.val > 99)
				setting.val = driverInfo_P->limitDS.startYMD[0];
			else if(setting.val < 22)
				setting.val = 99;
			if(setting.val < driverInfo_P->limitDS.startYMD[0])
			{
				driverInfo_P->limitDS.endYMD[0] = 99;
				setting.val = 99;
			}
			else
				driverInfo_P->limitDS.endYMD[0] = setting.val;
			break;		
		case SETTING_NOW_END_M:		//need larger than start month
			if(setting.val == 0xFF)
				setting.val = 11;
			else if(setting.val > 11)
				setting.val = 0;
			if(driverInfo_P->limitDS.endYMD[0] == driverInfo_P->limitDS.startYMD[0])//in same year
			{
				if(setting.val < driverInfo_P->limitDS.startYMD[1])
				{
					driverInfo_P->limitDS.endYMD[1] = 11;					
					setting.val = 11;
				}
				else
					driverInfo_P->limitDS.endYMD[1] = setting.val;
			}//under same year, end month need larger than start month	
			else
				driverInfo_P->limitDS.endYMD[1] = setting.val;
			//every change calculate days of this 			
			daysOfMonth = time_monthDayCount((month_t)driverInfo_P->limitDS.endYMD[1], \
				driverInfo_P->limitDS.endYMD[0]);
			if(driverInfo_P->limitDS.endYMD[2] > daysOfMonth)
				driverInfo_P->limitDS.endYMD[2] = daysOfMonth;
			break;		
		case SETTING_NOW_END_D:
			daysOfMonth = time_monthDayCount((month_t)driverInfo_P->limitDS.endYMD[1], \
				driverInfo_P->limitDS.endYMD[0]);
			if(setting.val > daysOfMonth)		//
				setting.val = 1;
			else if(setting.val < 1)
				setting.val = daysOfMonth;		//
			if(driverInfo_P->limitDS.endYMD[0] == driverInfo_P->limitDS.startYMD[0])
			{
				if(driverInfo_P->limitDS.endYMD[1] == driverInfo_P->limitDS.startYMD[1])
				{
					if(setting.val < driverInfo_P->limitDS.startYMD[2])
					{
						driverInfo_P->limitDS.endYMD[2] = daysOfMonth;
						setting.val = daysOfMonth;
					}
					else
						driverInfo_P->limitDS.endYMD[2] = setting.val;
				}
			}//under same year & same month end month need larger than start month
			driverInfo_P->limitDS.endYMD[2] = setting.val;
			break;
		default:
			break;
	}
}

void checkDataValid(void)
{
	byte cnt;
	if(driverInfo_P->limitDS.normal > 1)
		driverInfo_P->limitDS.normal = 0;
	for(cnt = 0; cnt < 3; cnt++)
	{
		if(driverInfo_P->limitDS.endYMD[cnt] == 0xFF || driverInfo_P->limitDS.startYMD[cnt] == 0xFF)
			break;
	}
	if(cnt != 3) //????????0????
	{
		//????driverInfo_P->limitDS.startYMD[cnt]
		driverInfo_P->limitDS.startYMD[0] = timeDate.date.year;
		driverInfo_P->limitDS.startYMD[1] = timeDate.date.month;
		driverInfo_P->limitDS.startYMD[2] = timeDate.date.date;
		driverInfo_P->limitDS.endYMD[0] = timeDate.date.year;
		driverInfo_P->limitDS.endYMD[1] = timeDate.date.month;
		driverInfo_P->limitDS.endYMD[2] = timeDate.date.date;
	}
}















