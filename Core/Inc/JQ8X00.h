...
#define JQ8x00_BusyCheck	1		//忙检测，0表示不做忙检测，1表做忙检测
...
//串口函数重定义
#define JQ8x00_UART(pointer,len) HAL_UART_Transmit(&huart2,pointer,len,0xffff)        //pointer为指针数据传入数据首地址，len为要发送数据个数
...
//忙检测IO配置
#define JQ8x00_BUSY_RCC          __HAL_RCC_GPIOB_CLK_ENABLE       //时钟
