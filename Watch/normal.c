...
void watchface_normal()
{
	...
	//buttons_setFuncs(altitude_open, menu_select, my_menu_open);
...
static void drawHeadLine()	//日期|周一|单号行驶|D/S
{
	...
	// Get date string
	char buff[BUFFSIZE_DATE_FORMAT];
	sprintf_P(buff, PSTR("20%02hhu"),timeDate.date.year);//, day, timeDate.date.date, , 
	draw_string(buff,false,3,0);//2022		24pixel
	sprintf_P(buff, PSTR("%02d.%02hhu"),(byte)(timeDate.date.month)+1,timeDate.date.date);
	draw_string(buff,false,0,8);//08.06		32pixel
	
	draw_bitmap(32+2, 0, textDisplay[8], 16, 16, NOINVERT, 0);//周
	dayOfWeek = 8 + (byte)(timeDate.date.day) + 1;
	draw_bitmap(48+2, 0, textDisplay[dayOfWeek], 16, 16, NOINVERT, 0);//几
	draw_bitmap(64+2, 0, textDisplay[2], 16, 16, NOINVERT, 0);//限
	draw_bitmap(80+2, 0, numFont11x16[0], 11, 16, NOINVERT, 0);//1
	draw_bitmap(92+2, 0, numFont11x16[9], 11, 16, NOINVERT, 0);//6
	
	draw_bitmap(FRAME_WIDTH-16, 0, textDisplay[22], 16, 16, NOINVERT, 0);//限行模式标志
...
static display_t ticker()
{
	...
	data.w = FONT_SMALL2_WIDTH;		//11
	data.h = FONT_SMALL2_HEIGHT;	//16
	...
	data.y = TIME_POS_Y;
	data.w = MIDFONT_WIDTH;			//19
	data.h = MIDFONT_HEIGHT;		//24
	...
	if(milliseconds%3600>1000)   // 假装是半秒钟  30ms
